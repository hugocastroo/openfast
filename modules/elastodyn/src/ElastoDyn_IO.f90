...
!   INTEGER(IntKi)               :: NumOuts                                  ! Number of output channel names read from the file
   ...
   IF ( PathIsRelative( TwrFile ) ) TwrFile = TRIM(PriPath)//TRIM(TwrFile)
	...
	! Drive Train losses - Use Additional Drive Train losses parameters? (flag): HC
	...
	END IF
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!	  
	! DTLInpN - Number of rotor input torque steps (-): HC
	CALL ReadVar( UnIn, InputFile, InputFileData%DTLInpN, "DTLInpN", "Number of inputs to specify losses (-)", ErrStat2, ErrMsg2, UnEc)
	CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )
	IF ( ErrStat >= AbortErrLev ) THEN
		CALL Cleanup()
		RETURN
	...
	IF ( InputFileData%DTLInpN > SIZE(InputFileData%DTLInpTrq) ) THEN
		CALL SetErrStat( ErrID_Warn, 'Number of inputs to specify rotor torque steps'// &
			  TRIM(Num2LStr(SIZE(InputFileData%DTLInpTrq)))//'.', ErrStat, ErrMsg, RoutineName )
	IF ( ErrStat >= AbortErrLev ) THEN
		CALL Cleanup()
		RETURN
	END IF
		InputFileData%DTLInpN = SIZE(InputFileData%DTLInpTrq)
	END IF

	! DTLInpTrq - Rotor input torque steps HC
	CALL ReadAry( UnIn, InputFile, InputFileData%DTLInpTrq, InputFileData%DTLInpN, "DTLInpTrq", "Rotor input torque steps (N-m)", ErrStat2, ErrMsg2, UnEc)
	CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )
	IF ( ErrStat >= AbortErrLev ) THEN
		CALL Cleanup()
		RETURN
	END IF
	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	IF ( InputFileData%DTLInpN > SIZE(InputFileData%DTLTrq) ) THEN
		CALL SetErrStat( ErrID_Warn, 'Number of inputs to specify losses exceeds '// &
			  TRIM(Num2LStr(SIZE(InputFileData%DTLTrq)))//'.', ErrStat, ErrMsg, RoutineName )
	IF ( ErrStat >= AbortErrLev ) THEN
		CALL Cleanup()
		RETURN
	END IF
		InputFileData%DTLInpN = SIZE(InputFileData%DTLTrq)
	END IF

	! DTLTrq - Drive train mechanical losses steps HC
	CALL ReadAry( UnIn, InputFile, InputFileData%DTLTrq, InputFileData%DTLInpN, "DTLTrq", "Drive train mechanical losses steps (N-m)", ErrStat2, ErrMsg2, UnEc)
	CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )
	IF ( ErrStat >= AbortErrLev ) THEN
		CALL Cleanup()
		RETURN
